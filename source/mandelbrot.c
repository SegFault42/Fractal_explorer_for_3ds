...
{
	for (fractal->x = 1; fractal->x < W_UP; fractal->x+=1)
	{
		for (fractal->y = 1; fractal->y < H_UP; fractal->y+=1)
		{
			...
			while (fractal->z_r * fractal->z_r + fractal->z_i * fractal->z_i < 4 && fractal->i < fractal->i_max);
			{
				if (fractal->i == fractal->i_max)
					sf2d_draw_rectangle(fractal->x, fractal->y, 1, 1, RGBA8(rand(), rand(), rand(), 255));
					/*drawPixel(fb, fractal->x, fractal->y, 0x0066ff);*/
				/*else*/
				/*{*/
					/*sf2d_draw_rectangle(fractal->x, fractal->y, 2, 2, fractal->i * color / fractal->i_max);*/
